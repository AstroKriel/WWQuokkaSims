...
PROBLEM_NAME = "BrioWuShockTube"

## the shock is placed at a hardcoded x=0.5, with custom Dirichlet boundaries baked to the fixed
## left/right shock states (see `MHDShocktubeProblem::setCustomBoundaryConditions`); domain and
## boundary conditions must stay fixed for the shock to remain centered and the BCs to apply
...
def build_sim_params(
    *,
    ...
    return write_param_groups.SimParams(
        ...
        ## no `resistivity`: `MHDShocktubeProblem` doesn't opt into resistive physics (default `ResistivityModel::none`)
