...
##   correctness run at real user-chosen geometry/resolution/output/time-integration.
## `SlowWaveConvergence` asserts `mhd.resistivity == 0` and aborts otherwise, in both modes;
## no `resistivity` kwarg is exposed here (use `AlfvenWaveLinear` for resistivity validation).
...
def build_sim_params(
    *,
    ...
    correctness run. `stop_time`/`max_time_steps` are required (not defaulted): the correct
    `stop_time` is one full wave period, which depends on `num_modes_x/y/z`/`angle_between_k_b0`,
    so defaulting it would silently be wrong for any combo other than the one it was measured for.
    """
