...
def initialize_whole_domain_view(
    *,
    ...
    `amr_level=0` always reads the base level directly.
    """
...
def load_sarray(
    *,
    whole_domain_view: typing.Any,
    field_key: read_fields.FieldKey,
) -> numpy.ndarray:
    """Read one scalar field out of an already-built whole-domain view as a plain 3D array."""
    sarray_3d = numpy.asarray(whole_domain_view[field_key], dtype=numpy.float64)
    if sarray_3d.ndim != 3:
